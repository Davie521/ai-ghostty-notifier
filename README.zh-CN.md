...
阈值可以修改。默认 20 分钟后通知过期；回到会话标签页或提交新提问时会提前清除。
...
这样那几秒里触发的 hook 就无法再拉起一个会活过换包的旧版进程。新副本就位之前
（通常一两秒，最多约二十秒），hook 找不到运行时，什么都不做：这段时间里该出的通知
不会显示。安装失败或被中断时，可执行位会还回去，旧版照常工作。只有对安装器
`kill -9` 才可能让它停在关闭状态，重跑安装器即可恢复。
...
[配置契约](docs/native-hook-configuration.md)。
