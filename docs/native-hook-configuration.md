...
| --- | --- |
| `MIN_ELAPSED`, `SOUND_ELAPSED`, `TIMEOUT` | `GHOSTTY_NOTIFY_` names; ASCII nonnegative integers, defaults 180 / 600 / 1200 for missing, empty or invalid input. Timeout 0 means no automatic expiry; cancellation still stops owned processes. |
...
| `GHOSTTY_NOTIFY_BACKEND` | Missing or empty means `auto`. `auto` / `agent` try the ready resident; `terminal-notifier` selects that external backend; `alerter` prefers alerter. Unavailable `agent` falls back to terminal-notifier, matching the old implementation (its old comment incorrectly said otherwise). |
...
| `GHOSTTY_NOTIFY_ALERTER` | Empty uses executable discovery. Nonempty relative paths are resolved before handoff, so resident cleanup uses the same executable as delivery. |
...
| `GHOSTTY_NOTIFY_FOCUS_POLL` | External-backend monitor only. Invalid/nonpositive values use 1 second; positive intervals have a 0.1-second minimum. The resident backend uses activation events. |
